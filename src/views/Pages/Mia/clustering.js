...
//import Edit from './ModalSolution/edit';
//import Rating from './ModalSolution/rating';
//import ModalSolution from './ModalSolution/solution';
import sampleData from '../../Commons/jsonFile/sample/clustering.json';
...
class Clustering extends Component {
        ...
        this.setState({datas : newDatas},
            () => {
                Swal.fire({
                    title: 'Success',
                    icon: 'success',
                    text: 'Technology has been submited.',
    ...
    editSubmitBtn = (id, data) => {
        ...
        var lbl_priority = '';
        var lbl_impact = '';
        if(data.valueAdd) {lbl_valueAdd = data.valueAdd.label}
        if(data.priority) {lbl_priority = data.priority.label}
        if(data.impact) {lbl_impact = data.impact.label}
        this.setState({
            datas: this.state.datas.map(el => (el.id === id ? 
                Object.assign(el, { valueAdd: lbl_valueAdd, priority: lbl_priority, impact: lbl_impact, dt5: status })
                : el))
        },
        () => {
            Swal.fire({
    ...
    filterSubmitBtn = (data) => {
        this.setState({
            ...
            selectedOptionTp: data.tp,
            selectedOptionOwner: data.owner,
            selectedOptionStatus: data.status
        });
        var newDatas = this.state.orgData;
        var tempDatas = [];
        if(data.tp && data.tp.length > 0 && data.status && data.status.length > 0 && data.owner && data.owner.length > 0){
            newDatas = [];
            data.tp.forEach(row => {
                newDatas = newDatas.concat(this.state.orgData.filter(item => item.tp === row.label));
            });
        ...
        }else if(data.tp && data.tp.length > 0 && data.status && data.status.length > 0){
            newDatas = [];
            data.tp.forEach(row => {
                newDatas = newDatas.concat(this.state.orgData.filter(item => item.tp === row.label));
            });
        ...
        }else if(data.tp && data.tp.length > 0){
            newDatas = [];
            data.tp.forEach(row => {
                newDatas = newDatas.concat(this.state.orgData.filter(item => item.tp === row.label));
            });
        ...
        switch (action) {
            ...
            case 'edit':
                if(row.dt5 === 'Pending') {
                    ...
                    Swal.fire({
                        title: 'Already Add Technology Description',
                        icon: 'info',
                        text: 'Data already submited.',
                        showConfirmButton: false
                    })
        ...
        const columns = [
            ...
            // },
            {
                ...
                formatter: (cell, row, rowIndex) => {
                    ...
                    return <span style={{color: clr, fontWeight: fw}}>{row.status}</span>;
                },
                headerStyle: (colum, colIndex) => {
                    return { width: '120px' };
                },
            },
            {
                ...
                formatter: (cellContent, row) => {
                    return (
                        <div>
                            ...
                            <span className="btnInTable">
                                <Button className="btn-google-plus btn-brand btn-sm icon" id="btn1" onClick={(e) => this.handleAction(e, row, 'delete')}><i className="icon-trash-alt"> </i></Button>
                    ...
                    {*/}
                            <span className="btnInTable">
                                <Button className="btn-twitter btn-brand btn-sm icon" id="btn2" onClick={(e) => this.handleAction(e, row, 'edit')}><i className="icon-pencil"> </i></Button>
                            ...
                            <span className="btnInTable">
                                sasa                              
                                <Button className='btn-vine btn-brand btn-sm icon' id='btn2' onClick={(e) => this.handleAction(e, row, 'approve')}><i className="icon-file-check"> </i></Button>

                                <UncontrolledTooltip placement="top" target="btn2">
                                Approval11 {row.status}
                                </UncontrolledTooltip>
                            ...
                            <span className="btnInTable">
                                ...
                                <Button className='btn-vine btn-brand btn-sm icon' id='btn3' onClick={(e) => this.handleAction(e, row, 'review')}>
                                    <i className="icon-file-check"> </i></Button>
                                    sasa
                            ...
                            <span className="btnInTable">
                                <Button className="btn-twitter btn-brand btn-sm icon" id="btn4" onClick={(e) => this.handleAction(e, row, 'preview')}><i className="icon-search4"> </i></Button>
                        ...
                        <Col xs="12" md="12">
                            <Card style={{marginBottom:'0'}}>
                                <CardBody>
                                    <Row>
                                        <Col md="12">
                                            <div className="titleFilter"><i className="icon-stack"></i> Collected and validated Cluster</div>
